...
import { IconsPlay } from "./iconsweb"; // Updated import
...
const AnimeCard: React.FC<AnimeCardProps> = ({ anime }) => {
  return (
    <Link
      to={`/anime/${anime.id}`}
      className="group cursor-pointer border-black border-2 bg-white h-fit w-full max-w-[250px] relative rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105"
    >
      <div className="flex w-full items-center justify-center relative">
        ...
        <div className="flex items-center flex-col p-2 absolute w-full h-full bg-gradient-to-b from-transparent via-transparent to-black justify-center transition-opacity duration-300 opacity-0 group-hover:opacity-100">
          <div className="flex items-center justify-center absolute left-2 top-2">
            <RatingProgress rating={anime.rating} />
          </div>
